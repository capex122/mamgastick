...
async function documentAt(url) { const r = await harbor.http(url, { headers: { "user-agent": "Mozilla/5.0" } }); return r.ok ? harbor.parseHtml(r.body) : null; }
function parseCards(doc) {
  if (!doc) return [];
  const result = [], seen = new Set();
  for (const card of doc.querySelectorAll(".card, .manga-card, .series-card, .row .col, article")) {
    const link = card.querySelector("a[href*='/series/']");
    const href = link && absolute(link.attr("href"));
    const id = seriesSlug(href);
    const heading = card.querySelector("h1, h2, h3, h4, h5, .title, .card-title");
    const title = (heading && heading.text()) || (link && link.text());
...
const plugin = {
  ...
  async search(query, offset) { const page = Math.floor(offset / PAGE_SIZE) + 1; return parseCards(await documentAt(`${BASE}/series?search=${encodeURIComponent(query)}&page=${page}`)); },
  async detail(id) {
  ...
  async chapters(id) {
    ...
    for (const link of doc.querySelectorAll(`a[href*='/series/']`)) {
      ...
      if (!url || !new RegExp(`/series/[^/]+/[^/?#]+`).test(new URL(url).pathname) || seen.has(url)) continue;
  ...
  async pageUrls(chapterId) {
    ...
    const result = [], seen = new Set();
    for (const img of doc.querySelectorAll(".chapter-content img, .reading-content img, .episode-content img, main img")) {
